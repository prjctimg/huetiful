...
export default (colors = [], startHue = 200, endHue = 300, mode = 'lch') => {
    var getColor = _.nth;

    // Color space and channel to query from.
    var colorArr = purify(colors);
    var colorObjArr = _.map(colorArr, (val, key) => _.fromPairs([['hue', getChannel(val)(mode)], ['name', getColor(colors, key)]]));

    // This variable stores the array that matches the filtering criteria defined by the start and end hues
    var filteredArr = _.filter(colorObjArr, (val) => _.inRange(val.hue, startHue, endHue)
