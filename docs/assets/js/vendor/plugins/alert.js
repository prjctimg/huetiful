...
(function () {
  ...
  const install = function (hook, vm) {
    const options = mergeObjects(
      CONFIG,
      vm.config['flexible-alerts'] || vm.config.flexibleAlerts
    );

    const findSetting = function findAlertSetting(
      input,
      key,
      fallback,
      callback
    ...
    hook.afterEach(function (html, next) {
      const modifiedHtml = html.replace(
        ...
        function (match, key, settings, value) {
          if (
            !options[key.toLowerCase()] &&
            options.typeMappings[key.toLowerCase()]
          ) {
          ...
          let isIconVisible = findSetting(
            settings,
            'iconVisibility',
            'visible',
            (value) => value !== 'hidden'
          );
          let isLabelVisible = findSetting(
            settings,
            'labelVisibility',
            'visible',
            (value) => value !== 'hidden'
          ...
          return `<div class="alert ${style} ${className}">
