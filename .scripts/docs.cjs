...
function fileContents(rootDir, filenameSegment, extension = 'md') {
  return readFileSync(
    `../docs/${rootDir}/${filenameSegment}.${extension}`,
    'utf8'
  );
}
...
function generateDocs(filenameSegment) {
  let typeDocMarkdownOutput = fileContents(`temp/modules`, filenameSegment);
  let templateMarkdownFile = fileContents('templates', filenameSegment);
  if (templateMarkdownFile.match(reHtmlComment)) {
    ...
    writeFileSync(
      `../docs/${filenameSegment}.html`,
      // This is the source .md file in ./docs we're writing to
