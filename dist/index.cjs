...
var cb3 = (iterations, distance, color) => samples_default(iterations).map(
  ...
  scheme = scheme.toLowerCase(), easingFunc = checkArg(easingFunc, easingSmoothstep), color = useMode(definition_default8)(color);
...
var earthtone = (color, options) => {
  ...
  let base2 = {
    ...
    "dark-brown": "#473b31",
    dark: "#352a21"
  }[earthtones.toLowerCase()], f3 = interpolate([base2, toHex(color), easingFunc], "lch", {
    h: {
      fixup: hueFixup,
      use: hueInterpolator
    },
...
var sortByDistance = (colors2, against, order, mode2, weights) => sortedArr("distance", ((against2, mode3) => (color) => differenceEuclidean(mode3 || "lch", weights || [1, 1, 1, 0])(against2, color))(against, mode2), order)(colors2);
...
), xyzConverter = (0, import_ciebase_ts.xyz)(import_ciebase_ts.workspace.WideGamutRGB, import_ciebase_ts.illuminant.D65), colorToCam = (color) => baseCieCam.fromXyz(
