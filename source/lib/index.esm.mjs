...
var colorObj = (factor3, callback) => (color2) => {
  return { [factor3]: callback(color2), color: color2 };
};
...
var filteredArr = (factor3, cb4) => (colors2, start, end) => {
  ...
  if (typeof start === "number") {
    result = colorObjArr(
      factor3,
      cb4
    )(colors2).filter((color2) => inRange(color2[factor3], start, end)).map((color2) => color2["color"]);
    ...
    const mapFilter = (test) => {
      return colorObjArr(
        factor3,
        cb4
      )(colors2).filter((el) => test(el[factor3], parseFloat(val))).map((el) => el["color"]);
    };
    switch (op) {
      case "<":
        result = mapFilter(lt);
...
function filterByDistance(colors2, against, startDistance = 0.05, endDistance, mode2, weights) {
  ...
  result = result.concat(
    ...filteredArr(factor3, cb4(against, mode2))(
      colors2,
      startDistance,
      endDistance
    )
...
function filterByContrast(colors2, against, startContrast = 0.05, endContrast) {
  ...
  result = result.concat(
    ...filteredArr(factor3, cb4(against))(colors2, startContrast, endContrast)
  );
  return result;
}
...
import { easingSmootherstep as easingSmootherstep2, modeLab, useMode as useMode9 } from "culori/fn";
var toLab = useMode9(modeLab);
...
var isCool = (color2) => {
  ...
  return predicate2(factor3, "cool");
};
var isWarm = (color2) => {
  const factor3 = getChannel("lch.h")(color2);
  return predicate2(factor3, "cool");
...
var getHueFamily = (color2, mode2) => {
  const factor3 = getChannel(`${mode2}.h`)(color2);
  return baseFunc22(factor3);
};
...
var overtone = (color2) => {
  const factor3 = getChannel("lch.h")(color2);
  let hue = customFindKey(hueTemperature_default, factor3);
