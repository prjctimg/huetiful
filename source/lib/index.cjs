...
function lightnessMapper(n3) {
  return (start1, end1) => (start2, end2) => (n3 - start1) / (end1 - start1) * (end2 - start2) + start2;
}
...
function interpolateSpline(colors2, mode2, samples2, kind, closed = false, options) {
  let {
...
function filterByHue(colors2, startHue = 0, endHue = 360) {
  ...
  result = result.concat(...filteredArr(factor4, cb4)(colors2, startHue, endHue));
  return result;
}
