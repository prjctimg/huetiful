...
Whenever possible, the library passes internal defaults to make sure that the least amount of errors surface unnecessarily. For example all `options` objects are optional but in certain scenarios these options have to be specified to remove ambiguity.
